...
public class DetectionOfSigns implements Analysis {
    ...
    public ArrayList<ArrayList<String>> transferFunction(Edge edge, ArrayList<ArrayList<String>> elementDS) {
        ...
        if(edge.getProgramStep().getClass().getName().equals("ast.declaration.IntegerDeclaration")) {
            ...
            ArrayList<String> temp = new ArrayList<>();
            //TODO check size being negative (easy just get size)
            temp.add(edge.getProgramStep().getIdentifier());
            temp.add("0");
            for(ArrayList<String> arrayList : elementDS) {
                if(!arrayList.contains(edge.getProgramStep().getIdentifier())) {
            ...
            temp.add("0");
            for(ArrayList<String> arrayList : elementDS) {
                if(!arrayList.contains(edge.getProgramStep().getIdentifier() + ".fst")) {
                    result.add(arrayList);
                }
            }
            result.add(temp);
            ...
            temp2.add("0");
            for(ArrayList<String> arrayList : elementDS) {
                if(!arrayList.contains(edge.getProgramStep().getIdentifier() + ".snd")) {
                    result.add(arrayList);
                }
            }
            result.add(temp2);
            ...
            result.add(temp);
            ArrayList<String> temp2 = new ArrayList<>();
            temp2.add(edge.getProgramStep().getIdentifier());
            temp2.add("0");
            result.add(temp2);
            ArrayList<String> temp3 = new ArrayList<>();
            temp3.add(edge.getProgramStep().getIdentifier());
            temp3.add("+");
            result.add(temp3);
        }
        else if (edge.getProgramStep().getClass().getName().equals("ast.statement.WriteStatement")) {
            //TODO check array index
            //program stepe get a expresion ekle
            //edge.getProgramStep().getaExpression().getClass().getName().equals("ast.expression.aexpression.AArrayExpression)
            //check fonksiyon(getaExpression().getaExpression(), elementDS) üstküme {-}
        }
        else if (edge.getProgramStep().getClass().getName().equals("ast.statement.RecordAssignment")) {
            //TODO fonksiyona gonder
            for(ArrayList<String> arrayList : elementDS) {
            ...
            //TODO fonksiyona gonder
            //TODO check array index (lexpression)
            for(ArrayList<String> arrayList : elementDS) {
        ...
        else if(edge.getProgramStep().getClass().getName().startsWith("ast.expression.booleanExpression")) {
            result = elementDS;
        }
        return result;
    }
    ...
    public ArrayList<String> signFunction(AExpression aExpression, ArrayList<ArrayList<String>> elementDS) {
        ...
        if(aExpression.getClass().getName().equals("ast.expression.aexpression.AValueExpression")) {
            ...
            for(ArrayList<String> arrayList : elementDS) {
                if(arrayList.contains(aExpression.getIdentifier() + ".fst")) {
                    result.add(arrayList.get(1));
                }
            }
        }
            ...
            for(ArrayList<String> arrayList : elementDS) {
                if(arrayList.contains(aExpression.getIdentifier() + ".snd")) {
                    result.add(arrayList.get(1));
                }
            }
        }
